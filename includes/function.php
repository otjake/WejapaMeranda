...
//        $sql="SELECT posts.* FROM `posts` JOIN `headline` ON posts.post_code=headline.post_code WHERE headline.headline_status=1 AND posts.editor_pick=1 AND posts.posts_status=1 ORDER BY RAND () ASC LIMIT 0,$qty";
    ...
    function head_line_sought(){
        global $conn;
        $sql="SELECT DISTINCT headline.headline_name as headies,headline.post_code as post_code FROM `headline` JOIN posts ON headline.post_code=posts.post_code WHERE headline.headline_status=1 AND `headline_status`=1 ORDER BY headline.headline_id DESC";
        $sql_exec=mysqli_query($conn,$sql);
        return $sql_exec;
    }
...
function post_select($qty){
    global $conn;
    $sql="SELECT posts.* FROM `posts` JOIN `headline` ON posts.post_code=headline.post_code WHERE headline.headline_status=1 AND posts.editor_pick=0  ORDER BY RAND () ASC LIMIT 0,$qty";
